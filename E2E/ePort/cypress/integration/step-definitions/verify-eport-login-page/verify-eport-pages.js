...
Given("Verify ePort Login Page", () => {
  ...
  ePortLoginPage
    .allLinks()
    .eq(1)
    .should("have.text", "Need to Register for ePort?");
  ...
  ePortLoginPage
    ...
    .should("contain.text", "Read the User Guides");
...
Given("Verify ePort User Guides Page", () => {
  ...
  ePortHelpPage
    .allLinks()
    .eq(1)
    .should("have.text", "ePort User's Guide for sellers");
  ePortHelpPage
    .allLinks()
    .eq(2)
    .should("have.text", "ePort Organization Admin Guide");
  ePortHelpPage
    .allLinks()
    .eq(3)
    .should("have.text", "Getting around popup blocker in IE");
  ePortHelpPage.needHelpText().should("have.text", "Need Help?");
  ePortHelpPage
    .allLinks()
    .eq(4)
    .parent()
    .should("have.text", "Click here to submit a support case.");
...
Given("Verify ePort Forgot Password Page", () => {
  ...
  ePortReqPwdPage
    ...
    .should(
      "contain.text",
      "Please enter either your Email Address or your Username:"
    );
  ePortReqPwdPage.sEmail().should("contain.text", "Email Address:");
  ePortReqPwdPage.sUser().should("contain.text", "OR Username:");
  ePortReqPwdPage
    .sLowerMessage()
    .should("contain.text", "Your new password will be emailed to you.");
...
Given("Verify ePort Home Page", () => {
  ePortHomePage
    ...
    .should(
      "contain.text",
      `Welcome ${envUtils.getEPortUserFirstName()} ${envUtils.getEPortUserLastName()}!`
    )
    .should("contain.text", "You are viewing documents for")
    .should(
      "contain.text",
      `${envUtils.getEPortUserFirstName()} ${envUtils.getEPortUserLastName()}.`
    );
  ePortHomePage.quickSearchLink().then(($element) => {
    expect($element.text()).equal("Quick Search");
    cy.log(`Verifying "${$element.text()}" link`);
    cy.request("HEAD", $element.prop("href")).its("status").should("eq", 200);
  });
  ...
  ePortHomePage
    ...
    .should("contain.text", "C/T Allocations");
  ePortHomePage
    .inboxGridHeaders()
