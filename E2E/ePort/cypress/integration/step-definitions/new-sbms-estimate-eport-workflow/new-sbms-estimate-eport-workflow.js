...
Given("Search for order in Eleven", () => {
  ...
  cy.readFile(estNumFilePath).then(($estimateNumber) => {
    const waitForEstimate = () => {
      ...
      const waitForOrder = () => {
        cy.wait(500);
        cy.is_element_exists(elevenHomePage.headerEstimateTextSyntax()).then(
          ($elementExists) => {
...
Given("Send order to ePort", () => {
  ...
  elevenHomePage
    ...
    .select(envUtils.getSubRepOffice());
  elevenHomePage
    .selectChangeAEDropdown()
    .select(
      `${envUtils.getEPortUserLastName()}, ${envUtils.getEPortUserFirstName()} - spot seller`
    );
  ...
  cy.readFile(estNumFilePath).then(($estimateNumber) => {
    ...
    const waitForEstimate = () => {
      ...
      ePortHomePage
        ...
        .then(($firstRowText) => {
          if (
            !$firstRowText.includes($estimateNumber) &&
            startIndex < endIndex
          ) {
            cy.log(
              `Waiting on  ${$estimateNumber} order ${
                (startIndex + 1) * 5
              } seconds`
            );
            cy.wait(5000);
